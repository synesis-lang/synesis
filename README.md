...
[LICENSE](https://github.com/synesis-lang/synesis/blob/main/LICENSE) and
[LICENSE.exception](https://github.com/synesis-lang/synesis/blob/main/LICENSE.exception).

SPDX identifier: `AGPL-3.0-only AND LicenseRef-Synesis-data-output-exception`

**Your research data and compiled outputs are yours.** The files you feed to
Synesis and the artifacts it produces from them — JSON, CSV, Excel, REFI-QDA,
DOCX, Alpaca JSONL, and knowledge-graph exports — are **not** covered by the
AGPL and carry no copyleft obligation toward Synesis. You may license and use
those outputs however you wish. This holds even when an output carries
Synesis's own runtime material, such as the JavaScript and CSS embedded in a
generated graph visualization. See `LICENSE.exception` for the terms.

The AGPL applies to Synesis itself: if you modify Synesis and distribute it, or
...
they were issued under. This change does not affect any existing user's right
to use outputs they have already compiled.
